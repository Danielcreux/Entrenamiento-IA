...
# CONFIGURACIÓN
# ==========================
MODEL_NAME = "/kaggle/working/qwen25-05b-jvc"       # mismo modelo base que en entrenar.py
...
def build_prompt(tokenizer, user_text: str) -> str:
    """
    Construye el texto de entrada siguiendo la plantilla de chat de Qwen.
    Sistema: entrenador físico experto
    """
    if hasattr(tokenizer, "apply_chat_template"):
        messages = [
            {"role": "system", "content": "Eres un entrenador personal experto en fitness y ejercicios. Proporcionas recomendaciones claras, seguras y efectivas en español, adaptadas a las necesidades específicas de cada persona. Siempre enfatizas la importancia de la técnica correcta, la progresión gradual y la seguridad. Si no tienes suficiente información sobre alguna condición médica específica, recomiendas consultar con un profesional de la salud."},
        ...
        # fallback simple adaptado al contexto de entrenamiento
        return f"### Sistema: Eres un entrenador personal experto en fitness y ejercicios. Proporcionas recomendaciones claras, seguras y efectivas en español.\n\n### Usuario: {user_text}\n\n### Asistente:"
...
def main():
    ...
    print("Escribe una pregunta sobre ejercicios o entrenamiento (en español).")
    print("Ctrl+C o línea vacía para salir.\n")

    try:
        while True:
            user_text = input("💬 Tu pregunta: ").strip()
            if not user_text:
                print("👋 Saliendo.")
                break
            ...
            print("🏋️‍♂️ Respuesta del entrenador:\n")
            print(answer)
            print("\n" + "-" * 60 + "\n")
